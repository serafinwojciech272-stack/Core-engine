...
export function analyzeTrading(signals: EngineSignal[]): TradingAnalysis {
  ...
  const riskGate: TradingAnalysis["riskGate"] = volatilityPct >= 1.0 ? "BLOCK" : volatilityPct >= 0.35 ? "CAUTION" : "PASS";
  const directionalEdge = direction === "NEUTRAL" ? 0 : edgeScore;
  const confidence = clamp(0.50 + directionalEdge * 0.42 - (riskGate === "BLOCK" ? 0.16 : riskGate === "CAUTION" ? 0.06 : 0), 0.5, 0.92);
