...
export function analyzeTrading(signals: EngineSignal[]): TradingAnalysis {
  ...
  const baseRiskGate: TradingAnalysis["riskGate"] =
    volatilityPct >= 1.0 ? "BLOCK" : volatilityPct >= 0.35 ? "CAUTION" : "PASS";

  const directionalEdge = direction === "NEUTRAL" ? 0 : edgeScore;
  const confidence = clamp(
    0.50 + directionalEdge * 0.42 -
      (baseRiskGate === "BLOCK" ? 0.16 : baseRiskGate === "CAUTION" ? 0.06 : 0),
    0.5,
    0.92
  );
  ...
  const riskGate: TradingAnalysis["riskGate"] =
    risk.gate === "PASS" && baseRiskGate === "PASS"
      ? "PASS"
      : risk.gate === "BLOCK" || baseRiskGate === "BLOCK"
        ? "BLOCK"
        : "CAUTION";
  ...
  const actionable =
    riskGate !== "BLOCK" &&
    regime === "TREND" &&
    Math.abs(primary) > 0.08 &&
    confidence >= 0.62;

  const decision: TradingAnalysis["decision"] =
    actionable && direction === "LONG"
      ? "LONG_WATCH"
      : actionable && direction === "SHORT"
        ? "SHORT_WATCH"
        : "WAIT";
  ...
  return {
    ...
    probabilities: {
      ...
      p2R: Number(p2R.toFixed(3)),
      p3R: Number(p3R.toFixed(3)),
    },
    expectedR,
    riskGate,
