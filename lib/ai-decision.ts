...
function deterministic(signals: EngineSignal[], domain?: string, learning: LearningContext[] = []): DecisionPayload {
  const names = signals.map((s) => s.name);
  const verifiedLearning = learning.filter((item) => item.quality === "VERIFIED");
  const negativeLearning = learning.filter((item) => item.quality === "NEGATIVE");
  const learningAdjustment = Math.min(0.04, verifiedLearning.length * 0.02) - Math.min(0.04, negativeLearning.length * 0.02);
  const learningEvidence = learning.slice(0, 4).map((item) =>
    `learning[${item.quality}][${item.kpi}]: ${item.lesson}`
  );
  ...
  if (domain === "trading" || names.includes("instrument") || names.includes("market_regime")) {
    ...
    return {
      ...
      recommendation,
      confidence: Math.max(0, Math.min(1, analysis.confidence + learningAdjustment)),
      priority: analysis.confidence >= 0.75 ? "HIGH" : analysis.confidence >= 0.62 ? "MEDIUM" : "LOW",
      evidence: [...signals.map((s) => s.name + ": " + s.value + " [" + s.source + "]"), ...learningEvidence],
  ...
  if (names.includes("qualified_leads")) {
    return {
      ...
      confidence: Math.max(0, Math.min(1, 0.88 + learningAdjustment)), priority: "HIGH",
      evidence: [...signals.map((s) => s.name + ": " + s.value + " [" + s.source + "]"), ...learningEvidence]
    };
  }
  ...
  if (names.includes("order_backlog")) {
    return {
      ...
      confidence: Math.max(0, Math.min(1, 0.86 + learningAdjustment)), priority: "HIGH",
  ...
  return {
    ...
    confidence: Math.max(0, Math.min(1, 0.91 + learningAdjustment)), priority: "HIGH",
    evidence: signals.map((s) => s.name + ": " + s.value + " [" + s.source + "]")
  };
}
...
export async function buildDecision(signals: EngineSignal[], domain?: string, learning: LearningContext[] = []): Promise<Decision> {
  const fallback = deterministic(signals, domain, learning);
  // Trading safety-critical fields are generated by the deterministic Risk/Trading Engine.
  // Do not allow a generic LLM response to weaken or bypass the risk gate.
  const isTrading = domain === "trading" || signals.some((s) => ["instrument", "market_regime", "momentum_pct", "volatility_pct"].includes(s.name));
  if (isTrading) return { id: crypto.randomUUID(), ...fallback };
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: JSON.stringify({ signals, historical_learning: learning.slice(0, 10) }) }
        ]
      }),
