...
function deterministic(signals: EngineSignal[]): DecisionPayload {
...
export async function buildDecision(signals: EngineSignal[]): Promise<Decision> {
  const fallback = deterministic(signals);
