...
import {missions,events,recordMissionEvent,transitionMission,claimMemoryAction,type MissionState} from "@/lib/engine";
...
export async function GET(request:Request){
 ...
 if(storageMode()==="supabase"){
  try{
   const [m,e]=await Promise.all([listPersistedMissions(limit),listPersistedEvents(limit)]);
   return NextResponse.json({ok:true,missions:m,count:m.length,persistence:"supabase",durable:true,events:e});
...
export async function POST(request:Request){
 ...
 try{
  ...
  const id=String(b.id||""),action=String(b.action||""),key=String(b.idempotencyKey||"");
  ...
  if(storageMode()==="supabase"){
   const current=(await listPersistedMissions(100)).find(m=>m.id===id);
   if(!current)return NextResponse.json({ok:false,error:"MISSION_NOT_FOUND"},{status:404});
   const policy=evaluateMissionAction(action,current.state);
   if(!policy.allowed)return NextResponse.json({ok:false,error:"POLICY_DENIED",reason:policy.reason},{status:403});
   const claim=await claimPersistedAction(id,action,key);
   if(!claim.claimed)return NextResponse.json({ok:true,duplicate:true,mission:current,action,persistence:"supabase",durable:true});
   let learning=null;
   if(action==="learn"){
    if(!assessment||assessment.quality==="UNVERIFIED")return NextResponse.json({ok:false,error:"LEARNING_UNVERIFIED"},{status:422});
    learning=await recordPersistedLearning(id,buildLearningLesson(assessment,{missionObjective:current.objective,kpi:current.kpi}));
   }
   const r=await transitionPersistedMission(id,next,policy.actor);
   ...
   const updated={...current,state:r.to_state,executionCount:r.execution_count,updatedAt:new Date().toISOString()};
   return NextResponse.json({ok:true,mission:updated,action,assessment,learning,persistence:"supabase",durable:true});
  }
  const m=missions.get(id);
  if(!m)return NextResponse.json({ok:false,error:"MISSION_NOT_FOUND"},{status:404});
  ...
  const policy=evaluateMissionAction(action,m.state);
  if(!policy.allowed)return NextResponse.json({ok:false,error:"POLICY_DENIED",reason:policy.reason},{status:403});
  const updated=transitionMission(m,next);
  updated.executionCount=action==="execute"||action==="retry"?m.executionCount+1:m.executionCount;
  missions.set(id,updated);
  const event=recordMissionEvent({missionId:id,decisionId:updated.decisionId,eventType:"STATE_CHANGED",fromState:m.state,toState:updated.state,actorType:policy.actor,metadata:{...outcome,...assessment?{assessment}:{}}});
  if(action==="execute"||action==="retry")recordMissionEvent({missionId:id,decisionId:updated.decisionId,eventType:"EXECUTION_RECORDED",toState:updated.state,actorType:"system"});
  return NextResponse.json({ok:true,mission:updated,action,assessment,event,persistence:"in-memory-runtime",durable:false,warning:"Non-durable demo mode."});
