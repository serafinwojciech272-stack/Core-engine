    ...
    if(storageMode()==="supabase"){
      ...
      if(capabilityActionId){
        ...
        if(action==="approve"){
          ...
          await recordPersistedMissionOutcome(id,"CAPABILITY_APPROVED",{capabilityActionId,actor:policy.actor});
          const rr=await transitionPersistedMission(id,next,policy.actor);
          return NextResponse.json({ok:true,mission:{...current,state:rr.to_state,executionCount:rr.execution_count,updatedAt:new Date().toISOString()},action,capabilityActionId,capabilityApproval:{status:"APPROVED",persistence:"supabase"},persistence:"supabase",durable:true});
        }
        if((action==="execute"||action==="retry")&&capability.requiresApproval){
          return NextResponse.json({ok:false,error:"CAPABILITY_APPROVAL_TRACKING_REQUIRES_DURABLE_LEDGER",capabilityActionId,detail:"The mission itself is durable, but capability approval state is not yet read from a dedicated database ledger."},{status:503});
        }
        if(action==="execute"||action==="retry"){
          const receipt=executeCapabilityAction({actionId:capabilityActionId,approved:true});
          await recordPersistedMissionOutcome(id,"CAPABILITY_EXECUTED",receipt);
        }
      }
      ...
      return NextResponse.json({ok:true,mission:updated,action,assessment,learning,persistence:"supabase",durable:true,capabilityLifecycle:capabilityActionId?"PARTIAL_DURABILITY":"STANDARD"});
    }
